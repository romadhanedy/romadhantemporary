...
async function loadAsyncComponents (to, from, next) {
  ...
  try {
    ...
    this.$wrapper.$emit('routeChanged', to, from, err)
    next()
  }
}
...
async function render (to, from, next) {
  ...
  try {
    ...
    if (error.message === 'ERR_REDIRECT') {
      return this.$wrapper.$emit('routeChanged', to, from, error)
    }
    _lastPaths = []
    ...
    this.error(error)
    this.$wrapper.$emit('routeChanged', to, from, error)
    next()
  }
}
...
function fixPrepatch (to, ___) {
  ...
  Vue.nextTick(() => {
    ...
    if (triggerScroll) {
      ...
      window.$wrapper.$nextTick(() => {
        window.$wrapper.$emit('triggerScroll')
      })
    }
...
function nuxtReady (_app) {
  window.onWrapperReadyCbs.forEach((cb) => {
    if (typeof cb === 'function') {
      cb(_app)
    }
  })
  // Special JSDOM
  if (typeof window._onWrapperLoaded === 'function') {
    window._onWrapperLoaded(_app)
  }
  // Add router hooks
  router.afterEach((to, from) => {
    // Wait for fixPrepatch + $data updates
    Vue.nextTick(() => _app.$wrapper.$emit('routeChanged', to, from))
  })
}
